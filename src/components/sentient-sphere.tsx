...
/**
 * Shader strings moved to module scope to avoid recreation on each render
 */
const VERTEX_SHADER = `
  uniform float uTime;
  varying vec2 vUv;
...
const FRAGMENT_SHADER = `
  ...
  void main() {
    ...
    // Gold-tinted color
    vec3 color = vec3(intensity * 1.0, intensity * 0.85, intensity * 0.5);
...
function Sphere() {
  ...
  useFrame((state, delta) => {
    if (materialRef.current?.uniforms?.uTime && materialRef.current?.uniforms?.uMouse) {
